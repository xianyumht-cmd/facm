...
namespace FACM.League
{
    internal static class LeagueEfficiencyUiBridge
    {
        ...
        private const string ItemSetMenuName = "FACM.LeagueItemSet";
        ...
        private static void AttachWhenReady(object sender, EventArgs e)
        {
            ...
            var item = new ToolStripMenuItem(LeagueEfficiencyText.Get(UiTextCatalog.Load(), LeagueEfficiencyUiTextKeys.Menu))
            {
                Name = MenuName
            };
            item.Click += delegate { Open(owner); };
            var itemSetIndex = FindIndex(menu, ItemSetMenuName);
            var insertAt = itemSetIndex >= 0 ? itemSetIndex + 1 : Math.Min(10, menu.Items.Count);
