...
{
    internal static class LeagueLiveUiBridge
    {
        private const string MenuName = "FACM.LeagueLive";
        private const string PlayerMenuName = "FACM.LeaguePlayer";
        private static readonly FieldInfo TrayField = typeof(MainForm).GetField("_tray", BindingFlags.Instance | BindingFlags.NonPublic);
        private static LeagueLiveModule _module;
        private static MainForm _owner;
        private static bool _dialogOpen;
        private static bool _installed;

        public static void Install(LeagueLiveModule module)
        {
        ...
        private static void AttachWhenReady(object sender, EventArgs e)
        {
            ...
            var item = new ToolStripMenuItem(UiTextRuntime.Text(UiTextKeys.LeagueLiveMenu)) { Name = MenuName };
            item.Click += delegate { Open(owner); };
            var playerIndex = FindIndex(menu, PlayerMenuName);
            var insertAt = playerIndex >= 0 ? playerIndex + 1 : Math.Min(6, menu.Items.Count);
        ...
        private static void Open(MainForm owner)
        {
            ...
            try
            {
                using (var form = _module.CreateLiveForm(UiTextCatalog.Load()))
                {
                    form.TopMost = true;
