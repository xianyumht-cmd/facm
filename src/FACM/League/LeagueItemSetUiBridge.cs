...
{
    internal static class LeagueItemSetUiBridge
    {
        private const string MenuName = "FACM.LeagueItemSet";
        private const string ApplyMenuName = "FACM.LeagueBuildApply";
        ...
        private static void AttachWhenReady(object sender, EventArgs e)
        {
            ...
            var item = new ToolStripMenuItem(
                LeagueAdvisorText.Get(UiTextCatalog.Load(), LeagueItemSetUiTextKeys.Menu))
            {
                Name = MenuName
            };
            item.Click += delegate { Open(owner); };
            var applyIndex = FindIndex(menu, ApplyMenuName);
            var insertAt = applyIndex >= 0 ? applyIndex + 1 : Math.Min(9, menu.Items.Count);
        ...
        private static void Open(MainForm owner)
        {
            ...
            try
            {
                using (var form = _module.CreateItemSetForm(UiTextCatalog.Load()))
                {
                    form.TopMost = true;
