...
public sealed record FlyingHostBundlePreparation(
    string ExecutablePath,
    string BundleSha256,
    string PayloadDirectory,
    bool CacheHit);

public sealed class WindowsFlyingHostBundleStore
{
    public const string ResourceName = "FACM.Resources.FlyingHost.zip";
    public const string HashResourceName = "FACM.Resources.FlyingHost.sha256";

    private const string HostExecutableName = "FACM.FlyingHost.exe";
    private const string CompletionMarkerName = ".facm-flyinghost-complete";
    ...
    private FlyingHostBundlePreparation? _cachedPreparation;

    public WindowsFlyingHostBundleStore(
        ...
        TimeSpan? prepareTimeout = null)
    {
        ...
        if (_prepareTimeout <= TimeSpan.Zero)
            throw new ArgumentOutOfRangeException(nameof(prepareTimeout), "FlyingHost preparation timeout must be positive.");
    }

    public async Task<FlyingHostBundlePreparation> PrepareAsync(CancellationToken cancellationToken = default)
    {
        if (!await _prepareGate.WaitAsync(_prepareTimeout, cancellationToken).ConfigureAwait(false))
        {
            ReportStage("prepare-gate-timeout");
            throw new TimeoutException($"FlyingHost payload preparation gate exceeded {_prepareTimeout.TotalSeconds:0.###} seconds.");
        }
        ...
        try
        {
            ...
            catch (TimeoutException)
            {
                ...
                throw new TimeoutException($"FlyingHost payload preparation exceeded {_prepareTimeout.TotalSeconds:0.###} seconds.");
            }
    ...
    private FlyingHostBundlePreparation PrepareCore(CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var bundleRoot = Path.Combine(_layout.RuntimeDirectory, "flying-host");
        ...
        if (_expectedBundleSha256 is { } expected)
        {
            ...
            if (IsComplete(expectedDestination, expectedExecutable, expectedMarker, expected))
            {
                ...
                return new FlyingHostBundlePreparation(expectedExecutable, expected, expectedDestination, CacheHit: true);
            }
        ...
        if (IsComplete(destination, executable, marker, bundleSha256))
        {
            ...
            return new FlyingHostBundlePreparation(executable, bundleSha256, destination, CacheHit: true);
        }
        ...
        if (Directory.Exists(destination))
        {
            ...
            {
                throw new IOException("Unable to clean incomplete FlyingHost payload directory.", exception);
            }
        }
        ...
        try
        {
            ...
            if (!File.Exists(stagedExecutable) || new FileInfo(stagedExecutable).Length < 65536)
                throw new InvalidDataException("Embedded FlyingHost payload does not contain a valid FACM.FlyingHost.exe.");
            ...
            if (fileCount < 1 || payloadBytes < 65536)
                throw new InvalidDataException("Embedded FlyingHost payload statistics are invalid.");
            ...
            return new FlyingHostBundlePreparation(executable, bundleSha256, destination, CacheHit: false);
        }
        catch
        {
            try
    ...
    private Stream OpenRequiredBundle() =>
        _openBundle() ?? throw new FileNotFoundException(
            "The FACM build does not contain the controlled FlyingHost payload resource.",
            ResourceName);
    ...
    private void ExtractArchive(Stream bundle, string stage, CancellationToken cancellationToken)
    {
        ...
        if (archive.Entries.Count < 1) throw new InvalidDataException("Embedded FlyingHost payload is empty.");
        ...
        foreach (var entry in archive.Entries)
        {
            ...
            if (Path.IsPathRooted(relative))
                throw new InvalidDataException("FlyingHost payload contains an absolute path: " + entry.FullName);
            ...
            if (!output.StartsWith(stagePrefix, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("FlyingHost payload contains a path traversal entry: " + entry.FullName);
