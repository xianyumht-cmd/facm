...
$bootstrapCMake = Require-File 'src/FACM.Bootstrapper/CMakeLists.txt'
$bootstrapDefaults = Require-File 'src/FACM.Bootstrapper/BootstrapDefaults.h.in'
...
$singleLauncherTest = Require-File 'tools/release/Test-FacmSingleLauncher.ps1'
...
foreach ($marker in @('CompiledDefaultBootstrapConfig', 'bootstrap-default-selected', 'bootstrap-config-invalid-fallback')) {
    if ($bootstrapper -notmatch [regex]::Escape($marker)) { throw "Bootstrapper is missing single-launcher marker: $marker" }
}
...
foreach ($marker in @('SingleLauncherBeforeFirstLaunch', 'EmbeddedCanonicalBootstrap', 'NoBootstrapLocalProvisioning', 'OptionalValidBootstrapOverride', 'MalformedBootstrapSafeFallback', 'BootstrapHttpDowngradeRejected', 'BootstrapTrustKeyInjectionIgnored', 'UnsignedProductionManifestRejected', 'RealFacmOrbLaunch')) {
    if ($singleLauncherTest -notmatch [regex]::Escape($marker)) { throw "Single-launcher test is missing assertion: $marker" }
}
...
foreach ($relative in @('tools/release/Prepare-FacmFreeDistCandidate.ps1', 'tools/release/Test-FacmFreeDistProxyTransport.ps1', 'tools/release/Test-FacmSingleLauncher.ps1', 'scripts/check-facm4-free-dist.ps1')) {
