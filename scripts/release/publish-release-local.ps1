...
param(
    ...
    [string]$Facm4ReleaseRoot = 'D:\project2\facm-release-4.0.1-selfsigned\release-assets',
    [string]$Facm4Version = '4.0.1',
    [string]$TargetCommit = '',
    [switch]$Publish
)
...
Require ($Facm4Version -match '^4\.0\.[0-9]+$') "Facm4Version must be a 4.0.x version: $Facm4Version"
Require (Test-Path -LiteralPath (Join-Path $LegacyReleaseRoot 'FACM.exe') -PathType Leaf) '3.5.17 FACM.exe is missing.'
Require (Test-Path -LiteralPath (Join-Path $Facm4ReleaseRoot 'FACM.exe') -PathType Leaf) "$Facm4Version FACM.exe is missing."
Require (Test-Path -LiteralPath (Join-Path $Facm4ReleaseRoot 'manifest.json.sig') -PathType Leaf) "$Facm4Version application signature is missing."
...
Require ($facm4Info.FileVersion -eq '4.0.0.0') "$Facm4Version bootstrapper PE version must remain 4.0.0.0: $($facm4Info.FileVersion)"
...
Require ([string]$versionManifest.migration.version -ceq $Facm4Version) "Migration version is not $Facm4Version."
Require ([string]$versionManifest.migration.manifest_url -ceq "https://github.com/xianyumht-cmd/facm/releases/download/v$Facm4Version/manifest.json") "Migration manifest URL is not v$Facm4Version."
...
Write-Host "$Facm4Version bootstrapper SHA-256: $(Get-Sha256 (Join-Path $Facm4ReleaseRoot 'FACM.exe'))"
Write-Host "$Facm4Version manifest SHA-256: $(Get-Sha256 (Join-Path $Facm4ReleaseRoot 'manifest.json'))"
...
& gh release create "v$Facm4Version" @facm4Assets `
    --repo xianyumht-cmd/facm --target $target --title "FACM $Facm4Version" `
    --notes "FACM $Facm4Version 自签名组件发布，使用最终本地源码重新构建。旧版用户请先通过 3.5.17 自动迁移；新用户可下载 FACM.exe 与 bootstrap.json。"
if ($LASTEXITCODE -ne 0) { throw "Creating v$Facm4Version GitHub Release failed." }

Write-Host "GitHub Releases v3.5.17 and v$Facm4Version created successfully."
