...
The current execution batch is the **challenge-token cleanup audit**. Akari removes tokens through the same challenge-preferences route with `challengeIds: []` and forwards its current banner selection. Because the same endpoint can behave as a replacement-style preference update, FACM must reuse the preservation-safe challenge-preferences owner established for the last-season banner instead of copying a narrow two-field POST.

The required FACM contract is: authoritative local preference pre-read, preserve current title/banner/crest/prestige fields, set only the selected challenge-token list to empty, issue one user-directed POST, then use bounded readback to prove the token list is empty while the preserved preference fields remain unchanged. Missing reconstruction/readback evidence must fail closed or remain unverified; no background rewrite loop is allowed.

After challenge-token cleanup, the remaining order is emote cleanup, game-view/lobby utilities, then login-time signature/display-rank reapply only if the shared League lifecycle can be reused safely. Exact status and boundaries live in `docs/RUNTIME-COMPANION-PROGRESS.md`.
