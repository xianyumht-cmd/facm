...
## 2026-09-02：旧版单文件更新器不能直接替换为 4.0 启动器

FACM 3.x 的更新器以 `FACM.exe` 进程在短时间内仍存活作为安装成功信号；FACM 4.0 的原生启动器可能在创建
`FACM.App.exe` 后立即退出。若直接把 4.0 `FACM.exe` 填入旧版清单，旧更新器会把正常的启动器退出误判为失败，或在
组件组合尚未提交时删除唯一回滚文件。
