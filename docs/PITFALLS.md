...
## 桌宠用了 WPF / deltaTime / 多帧，不代表已经避开旧 Sprite 失败路线

### 根因

PR #13 的旧 `SpritePetWindow` 已经具备 `Stopwatch + deltaTime`、约 16ms Tick、速度平滑、透明窗口、多方向动画和鼠标拖动；蜘蛛甚至使用过 8 方向 × 13 帧 Sprite。它仍被降级为回退/对照，核心问题不是缺少这些技术名词，而是“动画图片 + 窗口运动”没有形成可信的角色行为：固定步频与真实位移容易脱钩，巡航本质是随机目标速度，边缘直接翻转速度，转向也容易退化为方向行/镜像切换。

因此以后重新做桌宠时，如果只是把 WinForms 换成 WPF、把帧数加多、把 Timer 换成 Rendering 或继续调 FPS/随机速度，仍可能是同一失败方案的换皮。

### 防回归规则

- 新桌宠先做 **Gate 1 原地动作验收**：窗口不自动移动，只看 Idle / Walk / Run / Turn / Raised / Recover 等动作本身；原地动作不好看时禁止用桌面移动掩盖问题。
- 自动漫游必须后置到 Gate 2，并先用调试图形验证轨迹；Random 只选择行为/目标，不直接每帧改位置或速度。
- `actualSpeed` 必须驱动步频/Walk/Run；静止时不能继续走路动作，加减速时动作频率必须同步变化。
- 正式边缘行为禁止用 `vx = -vx` / `vy = -vy` 的弹球反射；应提前减速、观察、沿边或转向。
- 大角度转向不能把瞬间镜像/方向行切换当作完整 Turn；至少要有减速、转身过渡和重新加速语义。
- 编译、CI、FPS、CPU 低只能证明工程指标；是否有生命感必须由真实 Windows 视觉验收决定。
- 在独立 Prototype 视觉通过前，不接入 `FACM.exe` / `FACM.PetHost.exe`，也不替换已稳定的 VPet 正式路线。
