...
- Build #762：Shell 真正复位 + Sprite 桌宠取消硬屏幕边界；Windows Build 成功。用户已完成实机测试，确认“复位桌面位置”正常、Sprite 桌宠自然跑出屏幕后行为正常，两项均验收通过。
- 当前阶段：**Build #762 的复位与自由移动已实机验收通过；PR #40 仍未合并、未发布。** 蜘蛛方向行校准和苍蝇高清素材属于后续质量优化，不阻塞本次已验收结论。
...
蜘蛛方向行校准和苍蝇高清素材替换是当前桌宠质量优化的优先后续；Issue #33 的轻量蜘蛛方案仍作为相关独立方向保留。