...
- 启动卡属于 `FACM.PetHost/PetHostWindow.cs`，不是 VPet 配置项；真实进度来自 VPet 自身资源准备和 `LoadALL` 回调，不使用定时器模拟。
- 按最新产品文案，VPet 资源准备的 `x/1995` 阶段显示“正在编译着色器…” + determinate 进度条 + 百分比/完成数；“正在核对 VPet 官方动作清单”阶段改显示“加载中请稍等....”；第一次编译结束后的后续 `LoadALL` 动作/PNG 缓存阶段也显示“加载中请稍等....”并继续保留真实进度条。
- 按用户要求，加载卡不再显示“动画来源：VPet / VUP-Simulator（非商用授权）”，对应 TextBlock 已从 UI 中移除且卡片高度收紧；授权/来源信息继续保留在随包文档与 NOTICE 中。
...
- 用户后续视频表明加载窗口出现时间已明显恢复，但 `x/1995` 属于更早的 VPet 资源准备阶段；Build #716 已把该阶段接入真实进度条，Build #719 已验证移除动画来源 UI。
- 当前最新代码继续验证新的分阶段文案：只在第一次 `x/1995` 资源准备时显示“正在编译着色器…”，核对官方动作清单与后续 `LoadALL` 阶段显示“加载中请稍等....”。
