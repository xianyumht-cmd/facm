...
> 2026-08-11：FACM 3.1.2 已正式发布并启用在线更新；PR #40 正在实机验收 PetHost 启动卡与启动性能修复。
...
- 目标：把 VPet `LoadALL` 动作缓存阶段的 FACM 启动卡改为“正在编译着色器…” + determinate 真实进度条 + 百分比/`readyCount / graphCount`；该界面属于 `FACM.PetHost/PetHostWindow.cs`，不是 VPet 配置项。
- `VPet / VUP-Simulator（非商用授权）` 来源说明继续保留；“正在编译着色器”只是产品层展示文案，底层实际仍是 VPet 动作/PNG 缓存生成。
...
- 修复代码 HEAD `3e24aca7561273752dd65d719e54c86ce0c0843f` 的 FACM Windows Build #705 成功，Mayhem Source Probe #44 也成功；后续提交仅同步本任务的架构、状态与陷阱文档。
- 当前下一门槛：用最新任务分支 artifact 做真实 Windows **连续两次启动**验收。第一次新 bundle 允许发生一次后台释放；第二次必须明显命中快速缓存。两次都要确认动作缓存阶段实际显示“正在编译着色器…”和可见进度条。
