...
## 2026-09-02：迁移引导程序必须复用 3.5 Authenticode 身份并匹配补丁版本

### 决策

- 4.0 迁移引导程序必须使用与 3.5 bridge 相同的 Authenticode PFX；4.0 detached 清单签名仍继续使用独立 RSA 私钥。
- 原生资源的 numeric `FILEVERSION`、字符串 `FileVersion`/`ProductVersion` 和在线 migration 版本必须来自同一个 4.0.x 目标版本。
- 已发布的错误引导程序保留为历史 tag；修复发布使用新的补丁版本，不覆盖旧资产。

### 原因

旧版 bridge 在替换前会同时校验证书指纹和文件版本。只修清单签名或只更新在线版本号，都会在真实迁移时被拒绝。

### 后果

本地发布流程多一道 Authenticode 和资源版本核验，但可以在上传前捕获“未签名”及“4.0.0 文件冒充 4.0.x”两类失败。
