...
- The 2026-09-13 draft-aware counter extension intersects only the already-fetched OP.GG counter row with enemy champions already revealed in the same ChampSelect snapshot. Matching counters are reordered to the front only in the cloned Runtime Companion presentation; the Build Advisor owner's source snapshot and OP.GG source ordering remain untouched outside this surface. No hidden pick intent is used.
